...
public class MyBatisGenerator {
	...
	public MyBatisGenerator(Configuration configuration, ShellCallback shellCallback, List<String> warnings)
		...
		shellCallback.setWarnings(this.warnings);
		...
		try {
			...
			if (targetFile.exists()) {
				if (gjf.isMergeable()) {
					...
					warnings.add(getString("Warning.30", //$NON-NLS-1$
							gjf.getFileName()));
				} else if (gjf.isOverwriteEnabled()) {
					source = gjf.getFormattedContent();
					warnings.add(getString("Warning.11", //$NON-NLS-1$
							targetFile.getAbsolutePath()));
				} else {
					warnings.add(getString("Warning.29", //$NON-NLS-1$
							gjf.getFileName()));
					return;
				}
			} else {
