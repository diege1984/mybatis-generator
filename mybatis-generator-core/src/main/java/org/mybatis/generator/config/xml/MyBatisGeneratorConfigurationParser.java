...
public class MyBatisGeneratorConfigurationParser {
	...
	private void parseContext(Configuration configuration, Node node) {
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			} else if ("javaDTOGenerator".equals(childNode.getNodeName())) {
				parseDTOGenerator(context, childNode);
	...
	protected void parseTable(Context context, Node node) {
		...
		String clientTargetPackage = tc.getProperty("clientTargetPackage");
		String modelTargetPackage = tc.getProperty("modelTargetPackage");
		String mapTargetPackage = tc.getProperty("mapTargetPackage");
		String serviceTargetPackage = tc.getProperty("serviceTargetPackage");
		String dtoTargetPackage = tc.getProperty("dtoTargetPackage");
		
		if (stringHasValue(serviceTargetPackage)) {
			tc.setServiceTargetPackage(serviceTargetPackage);
		}
		
		if (stringHasValue(dtoTargetPackage)) {
			tc.setDtoTargetPackage(dtoTargetPackage);
		}
		
		if (stringHasValue(clientTargetPackage)) {
			tc.setClientTargetPackage(clientTargetPackage);
		}

		if (stringHasValue(modelTargetPackage)) {
			tc.setModelTargetPackage(modelTargetPackage);
		}

		if (stringHasValue(mapTargetPackage)) {
			tc.setMapTargetPackage(mapTargetPackage);
		}
	}
	...
	protected void parseDTOGenerator(Context context, Node node) {
		JavaDTOGeneratorConfiguration configuration = new JavaDTOGeneratorConfiguration();
		context.setJavaDTOGeneratorConfiguration(configuration);
		NodeList nodeList = node.getChildNodes();
		for (int i = 0; i < nodeList.getLength(); i++) {
			Node childNode = nodeList.item(i);
	...
	protected void parseServiceGenerator(Context context, Node node) {
		JavaServiceGeneratorConfiguration configuration = new JavaServiceGeneratorConfiguration();
		context.setJavaServiceGeneratorConfiguration(configuration);
		NodeList nodeList = node.getChildNodes();
		for (int i = 0; i < nodeList.getLength(); i++) {
			Node childNode = nodeList.item(i);
	 ...
	 * envireonments.
	 * 
	 * If there is a name collision, system properties take precedence, followed
	 * by configuration properties, followed by extra properties.
	 * 
	 * @param key
	 * @return the resolved property. This method will return null if the
	 *         property is undefined in any of the sources.
	 */
