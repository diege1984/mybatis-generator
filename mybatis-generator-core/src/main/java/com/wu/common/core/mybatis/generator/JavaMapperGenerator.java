...
public class JavaMapperGenerator extends AbstractJavaClientGenerator {
	...
	public List<CompilationUnit> getCompilationUnits() {
		...
		String blobInterface = introspectedTable.getTableConfigurationProperty(PropertyRegistry.ANY_BLOB_INTERFACE);
		String primaryInterface = introspectedTable.getTableConfigurationProperty(PropertyRegistry.ANY_PRIMARY_INTERFACE);
		if (!stringHasValue(rootInterface)) {
			rootInterface = context.getJavaClientGeneratorConfiguration()
					.getProperty(PropertyRegistry.ANY_ROOT_INTERFACE);
		}
		if (!stringHasValue(blobInterface)) {
			blobInterface = context.getJavaClientGeneratorConfiguration()
					.getProperty(PropertyRegistry.ANY_BLOB_INTERFACE);
		}
		if (!stringHasValue(primaryInterface)) {
			primaryInterface = context.getJavaClientGeneratorConfiguration()
					.getProperty(PropertyRegistry.ANY_PRIMARY_INTERFACE);
		}
		...
		if (stringHasValue(rootInterface)) {
			...
			fqjt.addTypeArgument(recordType);
		...
		if (introspectedTable.getRules().generateSelectByPrimaryKey()) {
			FullyQualifiedJavaType fqjt = new FullyQualifiedJavaType(primaryInterface);
			FullyQualifiedJavaType primaryType = null;
			if(introspectedTable.getRules().generatePrimaryKeyClass()){
				primaryType = new FullyQualifiedJavaType(introspectedTable.getPrimaryKeyType());
			}else{
				List<IntrospectedColumn> introspectedColumns = introspectedTable
	                    .getPrimaryKeyColumns();
				primaryType = introspectedColumns.get(0)
                        .getFullyQualifiedJavaType();
			}
			fqjt.addTypeArgument(recordType);
			fqjt.addTypeArgument(primaryType);
			
			interfaze.addSuperInterface(fqjt);
			interfaze.addImportedType(fqjt);
			interfaze.addImportedType(primaryType);
		}
		
		if (introspectedTable.getRules().generateUpdateByExampleWithBLOBs()) {
			
			FullyQualifiedJavaType fqjt = new FullyQualifiedJavaType(blobInterface);
			fqjt.addTypeArgument(recordType);
			fqjt.addTypeArgument(exampleType);
